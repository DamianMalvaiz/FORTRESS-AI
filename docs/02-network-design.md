# Network Design — FORTRESS-AI / BNF

**Esquema de direccionamiento IP, segmentación y arquitectura de red**
...
| **Dependencias** | [01 — Project Charter](01-project-charter.md) |
...
Este documento define el direccionamiento IP y la segmentación de red de BNF para el proyecto FORTRESS-AI. Establece cinco zonas de red con niveles de confianza diferenciados, interconectadas por un firewall central pfSense que actúa como gateway de cada zona.

El diseño se basa en el rango privado `10.0.0.0/8` y aplica **VLSM (Variable Length Subnet Masking)** para dimensionar cada zona según su función y su crecimiento previsto, siguiendo prácticas estándar de arquitectura bancaria.

**Decisiones clave:**

- **5 zonas de red** con niveles de confianza diferenciados
- **Segmentación mediante VLANs + subredes** aisladas en pfSense
...
El esquema de direccionamiento de BNF se rige por siete principios que se aplican de forma consistente en cada zona. Estos principios están alineados con prácticas estándar de arquitectura de red bancaria y con guías de referencia como NIST SP 800-41 y CIS Controls v8.
...
```
Red:         10.0.1.0
Gateway:     10.0.1.1    (pfSense interface em0)
Broadcast:   10.0.1.15
...
|---|---|---|---|
| 10.0.1.1 | Gateway pfSense DMZ | Semana 3 | Interface em0 |
...
```
Red:         10.0.2.0
Gateway:     10.0.2.1    (pfSense interface em1)
...
```
Red:         10.0.3.0
Gateway:     10.0.3.1    (pfSense interface em2)
...
```
Red:         10.0.4.0
Gateway:     10.0.4.1    (pfSense interface em3)
Broadcast:   10.0.4.15
...
```
Red:         10.0.5.0
Gateway:     10.0.5.1    (pfSense interface em4)
Broadcast:   10.0.5.15
...
| 10.0.4.1 | fw-soc | SOC | pfSense interface em3 | — | 1 |
...
| 10.0.5.1 | fw-mgmt | Management | pfSense interface em4 + panel admin | — | 1 |
...
### 7.1 Mapa de interfaces

| Interface física | Zona | IP | Red conectada | VLAN ID (planificado) |
|---|---|---|---|---|
| em0 | DMZ | 10.0.1.1/28 | 10.0.1.0/28 | 10 |
| em1 | LAN | 10.0.2.1/24 | 10.0.2.0/24 | 20 |
| em2 | Servers | 10.0.3.1/25 | 10.0.3.0/25 | 30 |
| em3 | SOC | 10.0.4.1/28 | 10.0.4.0/28 | 40 |
| em4 | Management | 10.0.5.1/28 | 10.0.5.0/28 | 50 |
...
### 11.1 Defense in Depth aplicado

El direccionamiento IP es **la capa 2 (segmentación)** de una estrategia de defensa en profundidad que incluye:

| Capa | Implementación en BNF |
|---|---|
| 1. Perímetro | Reglas pfSense entre zonas |
| 2. Segmentación | Subredes + VLANs definidas en este documento |
...
Este documento cubre exclusivamente la capa 2.
...
Para esos vectores se depende de las otras capas de defensa (EDR, GPO, patching, seguridad física).
...
**Documento controlado — FORTRESS-AI · Network Design v1.0 · 21 de abril de 2026**
